...
atlas_images = {
    sex: {layer: image_uri(f"{sex}-{layer}.webp") for layer in ("skin", "muscles", "organs", "skeleton")}
    for sex in ("female", "male")
}
atlas_systems = {
    sex: {
        system: image_uri(f"{sex}-system-{system}.webp")
        for system in ("nervous", "circulatory", "respiratory", "digestive", "urinary", "skeletal", "muscular", "reproductive")
    }
    for sex in ("female", "male")
}
...
<div id="body-explorer">
  <style>
    ...
    .atlas-image{position:absolute;inset:0;width:100%;height:700px;object-fit:cover;object-position:center;z-index:1;transition:opacity .32s ease,filter .32s ease;filter:saturate(1.03) contrast(1.02)}
    #bodySvg{width:100%;height:700px;display:block;position:relative;z-index:2;opacity:0;pointer-events:none;transition:opacity .3s ease}.stage.system-mode #bodySvg{opacity:0;pointer-events:auto}.stage.system-mode .atlas-image{opacity:1}.anatomy{transition:opacity .35s ease,transform .35s ease;transform-origin:center}.hidden-layer{opacity:0;pointer-events:none;transform:scale(.985)}
  ...
  <div class="be-shell">
    ...
    <section class="be-panel stage" aria-label="Interactive simplified human anatomy diagram">
      ...
      <img id="atlasImage" class="atlas-image" alt="Realistic medical teaching model showing the selected anatomical layer">
  ...
  (()=>{
    ...
    const atlas={girl:{skin:'__FEMALE_SKIN__',muscles:'__FEMALE_MUSCLES__',organs:'__FEMALE_ORGANS__',skeleton:'__FEMALE_SKELETON__'},boy:{skin:'__MALE_SKIN__',muscles:'__MALE_MUSCLES__',organs:'__MALE_ORGANS__',skeleton:'__MALE_SKELETON__'}};
    const systemAtlas={
      girl:{nervous:'__FEMALE_SYSTEM_NERVOUS__',circulatory:'__FEMALE_SYSTEM_CIRCULATORY__',respiratory:'__FEMALE_SYSTEM_RESPIRATORY__',digestive:'__FEMALE_SYSTEM_DIGESTIVE__',urinary:'__FEMALE_SYSTEM_URINARY__',skeletal:'__FEMALE_SYSTEM_SKELETAL__',muscular:'__FEMALE_SYSTEM_MUSCULAR__',reproductive:'__FEMALE_SYSTEM_REPRODUCTIVE__'},
      boy:{nervous:'__MALE_SYSTEM_NERVOUS__',circulatory:'__MALE_SYSTEM_CIRCULATORY__',respiratory:'__MALE_SYSTEM_RESPIRATORY__',digestive:'__MALE_SYSTEM_DIGESTIVE__',urinary:'__MALE_SYSTEM_URINARY__',skeletal:'__MALE_SYSTEM_SKELETAL__',muscular:'__MALE_SYSTEM_MUSCULAR__',reproductive:'__MALE_SYSTEM_REPRODUCTIVE__'}
    };
    const systemFacts={nervous:'The nervous system is the body’s fast communication network: brain, spinal cord and branching nerves.',circulatory:'The circulatory system moves oxygen, nutrients, hormones and heat through arteries, veins and tiny capillaries.',respiratory:'The respiratory system carries air to the lungs, where oxygen enters the blood and carbon dioxide leaves it.',digestive:'The digestive system breaks food into nutrients, absorbs them and moves unused material out of the body.',urinary:'The urinary system filters the blood and balances water, salts and acidity while producing urine.',skeletal:'The skeletal system supports the body, protects organs, stores minerals and makes blood cells inside marrow.',muscular:'The muscular system produces movement, stabilises joints, maintains posture and helps generate body heat.',reproductive:'The reproductive system contains specialised organs involved in reproduction and changes during puberty.'};
    let sex='girl',layer=0,target=null,score=0,tries=0;
    const range=root.querySelector('#layerRange'),callout=root.querySelector('#callout'),stage=root.querySelector('.stage'),atlasImage=root.querySelector('#atlasImage');
    function updateAtlas(){const active=activeSystems();atlasImage.src=active.length?systemAtlas[sex][active[0]]:atlas[sex][['skin','muscles','organs','skeleton'][layer]];}
    function setLayer(n){layer=Number(n);layers.forEach((id,i)=>root.querySelector('#'+id).classList.toggle('hidden-layer',i!==layer));root.querySelector('#layerName').textContent=layerNames[layer];root.querySelector('#layerNum').textContent=`Layer ${layer+1} of 4`;root.querySelectorAll('.ticks span').forEach((x,i)=>x.classList.toggle('active',i===layer));root.querySelector('#factText').textContent=layerFacts[layer];root.querySelector('#statusText').textContent=statusLines[layer];root.querySelectorAll('.sys-btn').forEach(b=>b.setAttribute('aria-pressed','false'));updateAtlas();applySystems();}
    function setSex(next){sex=next;root.querySelectorAll('.gender').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.sex===sex)));root.querySelectorAll('.sex-girl').forEach(x=>x.classList.toggle('sex-off',sex!=='girl'));root.querySelectorAll('.sex-boy').forEach(x=>x.classList.toggle('sex-off',sex!=='boy'));updateAtlas();}
    function activeSystems(){return [...root.querySelectorAll('.sys-btn[aria-pressed="true"]')].map(b=>b.dataset.system)}
    function applySystems(){const active=activeSystems();stage.classList.toggle('system-mode',active.length>0);root.querySelectorAll('[data-system-group]').forEach(g=>{if(!active.length){g.classList.remove('dimmed');return;}g.classList.toggle('dimmed',!active.includes(g.dataset.systemGroup));});if(active.length){root.querySelector('#organLayer').classList.remove('hidden-layer');root.querySelector('#muscleLayer').classList.remove('hidden-layer');root.querySelector('#skeletonLayer').classList.remove('hidden-layer');root.querySelector('#skinLayer').classList.add('hidden-layer');root.querySelector('#statusText').textContent='Medical atlas: '+active[0][0].toUpperCase()+active[0].slice(1)+' system';root.querySelector('#factText').textContent=systemFacts[active[0]];}else{root.querySelector('#factText').textContent=layerFacts[layer];setLayerVisualOnly();}updateAtlas();}
    ...
    root.querySelectorAll('.sys-btn').forEach(b=>b.addEventListener('click',()=>{const wasOn=b.getAttribute('aria-pressed')==='true';root.querySelectorAll('.sys-btn').forEach(x=>x.setAttribute('aria-pressed','false'));b.setAttribute('aria-pressed',String(!wasOn));applySystems();}));
...
for token, uri in {
    "__FEMALE_SKIN__": atlas_images["female"]["skin"],
    "__FEMALE_MUSCLES__": atlas_images["female"]["muscles"],
    "__FEMALE_ORGANS__": atlas_images["female"]["organs"],
    "__FEMALE_SKELETON__": atlas_images["female"]["skeleton"],
    "__MALE_SKIN__": atlas_images["male"]["skin"],
    "__MALE_MUSCLES__": atlas_images["male"]["muscles"],
    "__MALE_ORGANS__": atlas_images["male"]["organs"],
    "__MALE_SKELETON__": atlas_images["male"]["skeleton"],
}.items():
    EXPLORER = EXPLORER.replace(token, uri)

for sex_key, model_key in (("female", "FEMALE"), ("male", "MALE")):
    for system_key, uri in atlas_systems[sex_key].items():
        EXPLORER = EXPLORER.replace(f"__{model_key}_SYSTEM_{system_key.upper()}__", uri)
