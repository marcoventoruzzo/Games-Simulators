...
def matrix_html(company_name: str, profit: float, balance: float) -> str:
    ...
    return f"""
    <svg xmlns="http://www.w3.org/2000/svg" width="800" height="500" viewBox="0 0 800 500" role="img" aria-label="Profitability and balance-sheet matrix positioning {name}" style="font-family:system-ui,sans-serif">
        <defs><filter id="shadow"><feDropShadow dx="0" dy="4" stdDeviation="5" flood-opacity=".22"/></filter></defs>
        <rect x="70" y="55" width="330" height="190" rx="18" fill="#DDF6EA"/><rect x="400" y="55" width="330" height="190" rx="18" fill="#E4EEFF"/>
        <rect x="70" y="245" width="330" height="190" rx="18" fill="#FFF1D5"/><rect x="400" y="245" width="330" height="190" rx="18" fill="#FFE1E5"/>
        <line x1="400" y1="55" x2="400" y2="435" stroke="#526174" stroke-width="2"/><line x1="70" y1="245" x2="730" y2="245" stroke="#526174" stroke-width="2"/>
        <text x="235" y="105" text-anchor="middle" font-size="24" font-weight="700" fill="#126449">Champions</text><text x="235" y="132" text-anchor="middle" font-size="15" fill="#376458">high profit · strong balance sheet</text>
        <text x="565" y="105" text-anchor="middle" font-size="24" font-weight="700" fill="#27599B">Sleeping Giants</text><text x="565" y="132" text-anchor="middle" font-size="15" fill="#486482">low profit · strong balance sheet</text>
        <text x="235" y="385" text-anchor="middle" font-size="24" font-weight="700" fill="#9A6500">Fragile Survivors</text><text x="235" y="412" text-anchor="middle" font-size="15" fill="#7A6741">high profit · weak balance sheet</text>
        <text x="565" y="385" text-anchor="middle" font-size="24" font-weight="700" fill="#A6374D">Distressed</text><text x="565" y="412" text-anchor="middle" font-size="15" fill="#82525C">low profit · weak balance sheet</text>
        <text x="52" y="48" font-size="14" font-weight="700" fill="#334155">STRONG BALANCE SHEET ↑</text><text x="52" y="462" font-size="14" font-weight="700" fill="#334155">WEAK BALANCE SHEET ↓</text>
        <text x="70" y="485" font-size="14" font-weight="700" fill="#334155">← HIGH PROFITABILITY</text><text x="730" y="485" text-anchor="end" font-size="14" font-weight="700" fill="#334155">LOW PROFITABILITY →</text>
        <circle cx="{x:.1f}" cy="{y:.1f}" r="18" fill="#111827" stroke="white" stroke-width="5" filter="url(#shadow)"/>
        <rect x="{max(76,min(570,x-80)):.1f}" y="{max(62,y-58):.1f}" width="160" height="32" rx="16" fill="#111827" filter="url(#shadow)"/>
        <text x="{max(156,min(650,x)):.1f}" y="{max(83,y-37):.1f}" text-anchor="middle" font-size="14" font-weight="700" fill="white">{name}</text>
    </svg>"""
...
profit_index,balance_index=strategic_coordinates(ratios,a["target_margin"])
...
with tab_eval:
    ...
    matrix_svg = matrix_html(company,profit_index,balance_index).encode("utf-8")
    matrix_uri = "data:image/svg+xml;base64," + base64.b64encode(matrix_svg).decode("ascii")
    st.image(matrix_uri, width="stretch")
    st.caption(f"Positioning indices: profitability {profit_index:.0f}/100 · balance-sheet strength {balance_index:.0f}/100. These are transparent teaching indices derived from the ratios below, not external ratings.")
...
- **Balance-sheet-strength index:** combines current ratio, cash/debt and net debt/EBITDA. It determines the vertical position in the matrix.
