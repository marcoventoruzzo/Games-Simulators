...
DEFAULT_COMPANY_INPUTS = {
    "company_input": "ExampleCo",
    "industry_input": "Manufacturing",
    "revenue_input": 100000.0,
    "ebitda_input": 15000.0,
    "net_income_input": 7000.0,
    "opex_input": 85000.0,
    "current_ratio_input": 1.40,
    "cash_input": 8000.0,
    "debt_input": 30000.0,
}

def reset_company_inputs() -> None:
    """Restore the original worked example without changing the market context."""
    for key, value in DEFAULT_COMPANY_INPUTS.items():
        st.session_state[key] = value

for input_key, default_value in DEFAULT_COMPANY_INPUTS.items():
...
def strategic_coordinates(ratios, target_margin):
    """Continuous 0–100 teaching indices, intentionally simpler than a credit model."""
    profit = 100 * (
        clamp(ratios["EBITDA margin"] / target_margin if target_margin else 0)
        + clamp(ratios["Net margin"] / .10)
        + clamp((1.0 - ratios["Cost-to-income"]) / .30)
    ) / 3
    leverage = ratios["Net debt / EBITDA"]
    leverage_score = 1 if leverage <= 2 else clamp((4 - leverage) / 2)
    balance = 100 * (
        clamp(ratios["Current ratio"] / 1.5)
        + clamp(ratios["Cash / debt"] / .30)
        + leverage_score
    ) / 3
    return profit, balance

def matrix_html(company_name: str, profit: float, financial: float) -> str:
    # High profitability is deliberately plotted on the left, following the user's matrix.
    x = 65 + (100-profit) * 5.7
    y = 320 - financial * 2.55
    name = html.escape(company_name.strip() or "Your business")
    return f"""
    <svg xmlns="http://www.w3.org/2000/svg" width="700" height="390" viewBox="0 0 700 390" role="img" aria-label="Profitability and financial-strength matrix positioning {name}" style="font-family:system-ui,sans-serif">
        <defs><filter id="shadow"><feDropShadow dx="0" dy="3" stdDeviation="4" flood-opacity=".20"/></filter></defs>
        <rect x="190" y="5" width="320" height="30" rx="15" fill="#111827"/><circle cx="211" cy="20" r="6" fill="white"/>
        <text x="226" y="25" font-size="13" font-weight="700" fill="white">{name}</text>
        <text x="492" y="25" text-anchor="end" font-size="11" fill="#D7E0EC">Profit {profit:.0f} · Financial strength {financial:.0f}</text>
        <rect x="65" y="48" width="285" height="136" rx="14" fill="#DDF6EA"/><rect x="350" y="48" width="285" height="136" rx="14" fill="#E4EEFF"/>
        <rect x="65" y="184" width="285" height="136" rx="14" fill="#FFF1D5"/><rect x="350" y="184" width="285" height="136" rx="14" fill="#FFE1E5"/>
        <line x1="350" y1="48" x2="350" y2="320" stroke="#68778A" stroke-width="1.5"/><line x1="65" y1="184" x2="635" y2="184" stroke="#68778A" stroke-width="1.5"/>
        <text x="84" y="76" font-size="18" font-weight="700" fill="#126449">Champions</text><text x="84" y="96" font-size="11" fill="#376458">high profit · strong finances</text>
        <text x="369" y="76" font-size="18" font-weight="700" fill="#27599B">Sleeping Giants</text><text x="369" y="96" font-size="11" fill="#486482">low profit · strong finances</text>
        <text x="84" y="292" font-size="18" font-weight="700" fill="#9A6500">Fragile Survivors</text><text x="84" y="311" font-size="11" fill="#7A6741">high profit · weak finances</text>
        <text x="369" y="292" font-size="18" font-weight="700" fill="#A6374D">Distressed</text><text x="369" y="311" font-size="11" fill="#82525C">low profit · weak finances</text>
        <text x="55" y="43" font-size="11" font-weight="700" fill="#334155">STRONG FINANCIAL STRENGTH ↑</text><text x="55" y="340" font-size="11" font-weight="700" fill="#334155">WEAK FINANCIAL STRENGTH ↓</text>
        <text x="65" y="365" font-size="11" font-weight="700" fill="#334155">← HIGH PROFITABILITY</text><text x="635" y="365" text-anchor="end" font-size="11" font-weight="700" fill="#334155">LOW PROFITABILITY →</text>
        <circle cx="{x:.1f}" cy="{y:.1f}" r="14" fill="#111827" stroke="white" stroke-width="4" filter="url(#shadow)"/>
        <circle cx="{x:.1f}" cy="{y:.1f}" r="4" fill="white"/>
    </svg>"""
...
st.warning(
    "**Educational-use warning.** This tool is based on reasonable assumptions consistent with common business "
    "and market practice, but it should be treated as an educational game. It does **not** constitute financial "
    "advice and does **not** replace a genuine, company-specific valuation conducted by qualified human experts.",
...
st.markdown(
    ...
    section[data-testid="stSidebar"] div[data-testid="stButton"] button {
        ...
        font-weight: 800;
        min-height: 3.2rem;
        box-shadow: 0 5px 14px rgba(214, 40, 40, 0.28);
    }
    ...
    @media (max-width: 768px) {
        ...
        button[data-testid="stExpandSidebarButton"]::after {
            ...
            background: #fff3f3;
            color: #9f1d1d;
            box-shadow: 0 2px 8px rgba(159, 29, 29, 0.12);
            font-size: 0.78rem;
            font-weight: 800;
            line-height: 1;
            white-space: nowrap;
...
with st.sidebar:
    ...
    st.button(
        ...
        on_click=reset_company_inputs,
        use_container_width=True,
        help="Restore the original ExampleCo inputs.",
    )
    company=st.text_input("Company name", key="company_input")
    industry=st.selectbox("Industry", INDUSTRIES, key="industry_input")
    revenue=st.number_input(f"Revenue ({symbol})", min_value=1.0, step=1000.0, key="revenue_input")
    ebitda=st.number_input(f"EBITDA ({symbol})", step=1000.0, key="ebitda_input")
    net_income=st.number_input(f"Net income ({symbol})", step=1000.0, key="net_income_input")
    opex=st.number_input(f"Operating costs ({symbol})", min_value=0.0, step=1000.0, key="opex_input")
    current_ratio=st.number_input("Current ratio (x)", min_value=0.0, step=.10, key="current_ratio_input")
    cash=st.number_input(f"Cash ({symbol})", min_value=0.0, step=1000.0, key="cash_input")
    debt=st.number_input(f"Total debt ({symbol})", min_value=0.0, step=1000.0, key="debt_input")
...
profit_index,financial_index=strategic_coordinates(ratios,a["target_margin"])
...
with tab_eval:
    ...
    st.subheader("Where does the business sit?")
    matrix_svg = matrix_html(company,profit_index,financial_index).encode("utf-8")
    matrix_uri = "data:image/svg+xml;base64," + base64.b64encode(matrix_svg).decode("ascii")
    st.image(matrix_uri, width=700)
    st.caption(f"Profitability {profit_index:.0f}/100: EBITDA margin versus the selected industry target, supported by net margin and cost efficiency. Financial Strength {financial_index:.0f}/100: net debt/EBITDA, current ratio and cash/debt combined. These are teaching indices, not external ratings.")
...
**Important:** this is a pedagogical first diagnostic, not an investment recommendation, fairness opinion, credit rating or substitute for a full forecast. Banks, insurers and many real-estate vehicles require sector-specific valuation methods. Comparable-company multiples, company-specific WACC, debt maturities, leases, pensions, minorities, exceptional items and accounting quality require separate work.
    ...
    st.markdown("""
- **Overall health score:** the simple average of eight diagnostic scores. Healthy/OK receives 100 points, Watch 60, Risk 20, and an input-coherence warning 40.
- **EBITDA margin:** EBITDA divided by revenue. It is compared with the selected sector's target margin.
- **Net margin:** net income divided by revenue; it captures interest, tax and other items that EBITDA omits.
- **Cost-to-income:** operating costs divided by revenue. Lower values leave more room for shocks and reinvestment.
- **Current ratio:** current assets divided by current liabilities; a rough short-term liquidity measure.
- **Cash/debt:** the immediately visible cash buffer relative to financial debt.
- **Gross debt/EBITDA:** years of current EBITDA represented by total debt before considering cash.
- **Net debt/EBITDA:** debt less cash divided by EBITDA; a simplified leverage indicator.
- **Opex vs EBITDA check:** tests whether revenue minus operating costs approximately equals reported EBITDA. A warning usually means inconsistent definitions or periods.
- **Profitability index:** combines EBITDA margin relative to its sector target, net margin and cost efficiency. It determines the horizontal position in the matrix; high profitability is on the left.
- **Financial Strength index:** combines net debt/EBITDA, current ratio and cash/debt. It determines the vertical position in the matrix and is a simplified liquidity-and-leverage indicator—not a credit rating or complete balance-sheet analysis.
- **DCF enterprise value:** present value of five projected unlevered cash flows plus a discounted terminal value. It values operations before debt and cash.
- **EV/EBITDA enterprise value:** current EBITDA multiplied by the selected sector's low, base and high benchmark multiples.
- **Blended enterprise value:** weighted average of DCF and EV/EBITDA values. The sector table determines the DCF weight; the multiple weight is the remainder.
- **Indicative equity value:** blended enterprise value minus total debt plus cash. It is not automatically the price of the shares because leases, pensions, minorities and other claims may require further adjustments.
- **Valuation gap:** percentage difference between the base DCF and base multiple values. A large gap signals assumption sensitivity or an unusual business profile.
- **Low/base/high range:** deliberately broad scenarios, not statistical confidence intervals.
    """)

st.markdown("---")
st.markdown(
    "<p style='text-align:center; font-weight:700; color:#334155;'>"
    "(c) Marco Ventoruzzo, 2026. Made with Codex, Python and available on GitHub."
    "</p>",
    unsafe_allow_html=True,
)
